package com.tesco.substitutions.commons.vertx.redis;
...
public class RedisAdapter {
    ...
    private static void pingRedisContinuously(RedisClient redisClient) {
        Single.just(new JsonObject()).observeOn(Schedulers.io()).map((r) -> {
            return redisClient;
        }).observeOn(Schedulers.io()).flatMap((r) -> {
            return redisClient.rxPing();
        }).timeout(DEFAULT_TIMEOUT_PING_REDIS_MILLISECONDS, TimeUnit.MILLISECONDS)
                .onErrorResumeNext(RedisAdapter::logAndPropagateError).retryWhen((errors) -> {
            return errors.delay(DEFAULT_RETRY_TIME_REDIS_SECONDS, TimeUnit.SECONDS);
        }).subscribe((result) -> {
            LOGGER.info("Successfully established Redis connection");
        }, (error) -> {
            LOGGER.info("Error pinging Redis", error);
        });
    }
